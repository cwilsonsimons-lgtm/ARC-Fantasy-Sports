...
export function preset(kind) {
  if (kind === 'wings') return [
    ...
    slot({ id:'noteA', label:'Note A',   side:'a', text:'{{note}}', x:.03, y:.645, w:.44, h:.20, font:'oswald', color:'#ffffff', size:.055, wrap:true, stroke:.003 }),
  ...
  if (kind === 'scoreboard') return [
    ...
    slot({ id:'noteA', label:'Note A',   side:'a', text:'{{note}}', x:.035, y:.470, w:.26, h:.18, font:'oswald', color:'#ffffff', size:.038, wrap:true, stroke:.002 }),
