...
written, the section stays read-only rather than lose it.
...
js/
  ...
  universe/     WWE Universe — model.js (data + rules), persist.js (storage),
                app.js (commit, sheets, page stack), views.js (tabs),
                pages.js (profiles), edits.js (their sheets), sheets.js
                (create, events, results), index.js (shell)
...
```
npm run test:universe     # data model + persistence, under Node — no browser (49 tests)
npm run check:universe    # the section driven in Chromium: typing, selects, confirms,
                          # profile pages and Back, moves, line-ups, corrections,
                          # a real export download and import upload, reload, geometry (65 checks)
```
